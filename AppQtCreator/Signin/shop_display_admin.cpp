...
#include "ui_shop_display_admin.h"
...
shop_display_admin::shop_display_admin(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::shop_display_admin)
{
    ui->setupUi(this);
...
void shop_display_admin::on_pushButton_Display_From_Database_clicked()
{
    ...
    if (QueryGetImage.exec()) {
        ...
        while (QueryGetImage.next()) {
            ...
            if (image.loadFromData(QByteArray::fromBase64(productImageFromDatabase), "JPG")) {
                ...
                connect(pushButton, &QPushButton::clicked, [this, productId]() {
                    ...
                    Query_subtotal.prepare("INSERT INTO Cart_Item (productId,cartId)  VALUES (:productId,:cartId)");
                    Query_subtotal.bindValue(":productId", productId);
                    Query_subtotal.bindValue(":cartId", CART_ID);
                    Query_subtotal.exec();
                });
                ++rowNumber;
            } else {
        ...
        ui->tableWidget_Image_LIst_From_Database->setColumnWidth(1,130);
...
void shop_display_admin::on_on_btn_show_Tshirt_clicked()
{
    QSqlQuery QueryGetImage(MyDB::getInstance()->getDBInstance());
    QueryGetImage.clear();
...
void shop_display_admin::on_on_btn_show_Jacket_clicked()
{
    QSqlQuery QueryGetImage(MyDB::getInstance()->getDBInstance());
    QueryGetImage.clear();
...
void shop_display_admin::on_on_btn_show_Polo_clicked()
{
    QSqlQuery QueryGetImage(MyDB::getInstance()->getDBInstance());
    QueryGetImage.clear();
...
void shop_display_admin::on_on_btn_show_Pant_clicked()
{
    QSqlQuery QueryGetImage(MyDB::getInstance()->getDBInstance());
    QueryGetImage.clear();
...
void shop_display_admin::on_on_btn_show_Short_clicked()
{
    QSqlQuery QueryGetImage(MyDB::getInstance()->getDBInstance());
    QueryGetImage.clear();
...
void shop_display_admin::on_on_pushButton_Search_clicked()
{
    // QSqlQuery QueryGetImage(MyDB::getInstance()->getDBInstance());
    // QueryGetImage.clear();
...
void shop_display_admin::on_on_pushButton_Menu_clicked()
{
...
void shop_display_admin::on_pushButton_3_clicked()
{
    hide();
    emit returnToLogin();
}
...
void shop_display_admin::on_btn_cart_clicked()
{
    hide();
    shoppingCart = new ShoppingCart();
    connect(shoppingCart, &ShoppingCart::returnShoppingDisplay, this, &shop_display_admin::show);
    shoppingCart->show();
}
